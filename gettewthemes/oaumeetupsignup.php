...
header("location:oaumeetupdatingstories.php");
exit();
}
...
//data input checking,validation and cleaning starts here 
  if (empty($name) || empty($username) || empty($institution) || empty($gender) || empty($email) || empty($phone) || empty($password)) {
    ...
    $errmsg.="<span class='w3-text-red w3-bold'>Password can only contain letters and numbers</span><br>";
    }
...
/*for javascript disabled users*/}elseif(!isset($_POST["clicked"]) && $_SERVER["REQUEST_METHOD"] == "POST"){
  
  //checking for input set starts here
  if(isset($_POST["oaumeetupname"])){
  $name = $signup->clean_input($_POST["oaumeetupname"]);
  }
  if(isset($_POST["oaumeetupusername"])){
  $username = $signup->clean_input($_POST["oaumeetupusername"]);
  }
  if(isset($_POST["institution"])){
  $institution = $signup->clean_input($_POST["institution"]);
  }
  if(isset($_POST["oaumeetupgender"])){
  $gender = $signup->clean_input($_POST["oaumeetupgender"]);
  }
  if(isset($_POST["oaumeetupemail"])){
  $email = $signup->clean_input($_POST["oaumeetupemail"]);
  }
  if(isset($_POST["oaumeetupphone"])){
  $phone = $signup->clean_input($_POST["oaumeetupphone"]);
  }
  if(isset($_POST["oaumeetuppass"])){
  $password = $signup->clean_input($_POST["oaumeetuppass"]);
  }
  //checking for input set ends here
  ...
  <!--login form starts here-->
		...
		<form name="oaumeetupsignupform" id="oaumeetupsignupform"class="w3-form  w3-container w3-center w3-animate-zoom w3-opacity w3-blue w3-card-4 w3-round w3-block" method="post"action="<?php echo htmlentities($_SERVER["PHP_SELF"])."#footer";?>"style="">
			...
			<label class="w3-label w3-bold"for="oaumeetupname">Name</label>
             <input maxlength="50" name="oaumeetupname"id="oaumeetupname"class="w3-input stafflog_input w3-border w3-round-large"type="text"placeholder="Name eg:John Doe" value="<?php echo $name;?>"required>
             <label class="w3-label w3-bold"for="oaumeetupusername">Username</label>
            <input maxlength="10" name="oaumeetupusername" id="oaumeetupusername"class="w3-input stafflog_input w3-border w3-round-large"type="text"placeholder="username"value="<?php echo $username;?>" required>
            ...
            <label for ="oaumeetupgender" class="w3-label w3-bold">Gender</label><br>
            <input type="radio" name="oaumeetupgender" class="w3-radio oaumeetupgender" value="male" <?php if(isset($gender) && $gender == "male") echo "checked";?> >Male
            <input type="radio" name="oaumeetupgender" class="w3-radio oaumeetupgender" value="female" <?php if(isset($gender) && $gender == "female") echo "checked";?>>Female
            <br>
           <!-- <select id="oaumeetupgender" class="w3-select w3-round-large" name="oaumeetupgender">
            ...
            <input maxlength="50" name="oaumeetupemail" id="oaumeetupemail"class="w3-input stafflog_input w3-border w3-round-large"type="email"placeholder="johndoe@example.com"value="<?php echo $email;?>"required>	
			      <label class="w3-label w3-bold"for="oaumeetupphone">Phone number</label>
            <p>By siging up you agree to our <b><a href='tcondition.php'class=''style="">terms & conditions</a></b></p>
            <input maxlength="11" name="oaumeetupphone" id="oaumeetupphone"class="w3-input stafflog_input w3-border w3-round-large"type="tel"placeholder="eg:0801233...."value="<?php echo $phone;?>" required>	
        <label class="w3-label w3-bold"for="oaumeetuppass">Create Password</label>
			<input maxlength="15" name="oaumeetuppass" id="oaumeetuppass"class="w3-input stafflog_input w3-round-large"type="password"placeholder="**********"value="<?php echo $password;?>" required>
...
<script type="text/javascript">
 ...
 $("#oaumeetupsignupform").submit(function(e){
    ...
    gender = $("input:radio[name='oaumeetupgender']:checked").val();
    email = $("#oaumeetupemail").val();
    phone = $("#oaumeetupphone").val();
    password = $("#oaumeetuppass").val();
    btn = $("#signupbtn");
    errctn = $("#errmsg");
    if($("input:radio[name='oaumeetupgender']:checked").length != 1){
    alert("You havent selected your gender yet");
    return;
    }
